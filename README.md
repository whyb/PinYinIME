...
| 维度 | v1.x (旧版) | v2.0 (当前) |
|------|-------------|-------------|
| 输入框架 | 全局键盘钩子 (`WH_KEYBOARD_LL`) | TSF (`ITfKeyEventSink`) |
| 进程模型 | 单 EXE | EXE + DLL (DLL 被注入到各应用) |
| 权限要求 | 必须管理员 | 普通用户即可输入，注册系统需管理员 |
| 词库加载 | 每进程独立解析 YAML | 首进程解析 → 共享内存 → 后续进程零拷贝映射 |
| 光标检测 | UIA + Win32 Caret | UIA + TSF `GetTextExt` + Win32 Caret 三源交叉验证 |
| 系统集成 | 无 | Windows 语言栏原生集成 |
| 注册方式 | 无 | COM + TSF Profile 注册 |
